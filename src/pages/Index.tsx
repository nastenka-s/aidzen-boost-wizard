...
// ВАЖНО: Мы сохраняем импорт внешнего компонента, который будет рисовать колесо
...
// ====================================================================
// 2. ИНТЕРФЕЙСЫ И ПОЛНАЯ ФУНКЦИЯ РАСЧЕТА МАТРИЦЫ
// ====================================================================
...
interface MatrixResult {
  ...
  karma_tail_triplet: ArcanaPoint[]; // Добавляем пустой массив для предотвращения ошибки .map()
  [key: string]: any;
}
...
const calculateFullMatrix = (date: Date): MatrixResult => {
  ...
  const W_num = r(B_num + L_num); // BL (Bottom-Left)
  ...
  const wheel_spokes = {
    // Внутренний круг (R_INNER): Угол + Центр
    L_inner: innerArcana(L_num, C_num),
    T_inner: innerArcana(T_num, C_num),
    R_inner: innerArcana(R_num, C_num),
    B_inner: innerArcana(B_num, C_num),

    // Внутренний квадрат
    N_inner: innerArcana(N_num, C_num),
    E_inner: innerArcana(E_num, C_num),
    S_inner: innerArcana(S_num, C_num),
    W_inner: innerArcana(W_num, C_num),

    // Средний круг: Угол + Внутренний
    L_middle: innerArcana(L_num, innerArcana(L_num, C_num).num),
    T_middle: innerArcana(T_num, innerArcana(T_num, C_num).num),
    R_middle: innerArcana(R_num, innerArcana(R_num, C_num).num),
    B_middle: innerArcana(B_num, innerArcana(B_num, C_num).num),

    // Средний квадрат
    N_middle: innerArcana(N_num, innerArcana(N_num, C_num).num),
    E_middle: innerArcana(E_num, innerArcana(E_num, C_num).num),
    S_middle: innerArcana(S_num, innerArcana(S_num, C_num).num),
    W_middle: innerArcana(W_num, innerArcana(W_num, C_num).num),
  };

  // ВНИМАНИЕ: Если MatrixVisualization.tsx не может работать без массива
  // "кармического хвоста", добавим его сюда (значения 22, 11, 10 - пример)
  const karma_tail_triplet: ArcanaPoint[] = [
    innerArcana(22, 0), // Примерные стартовые арканы
    innerArcana(11, 0),
    innerArcana(10, 0),
  ];
  ...
  return {
    ...
    W: { num: W_num, name: ARCANA_NAMES[W_num] }, // BL теперь W
    ...
    karma_tail_triplet: karma_tail_triplet, // Передаем для предотвращения ошибки map
  };
};
              ...
              <CardContent className="p-6 md:p-8">
                <div className="flex flex-col md:flex-row gap-8">
                  ...
                  <div className="flex-grow">
                    {/* КАРТОЧКА BL (ВОССТАНОВЛЕНА С КОРРЕКТНЫМ ИСПОЛЬЗОВАНИЕМ .num И .name) */}
                    {/* W (Bottom-Left) используется вместо matrix.BL */}
                    ...
                    {/* Ключевые точки (ВОССТАНОВЛЕНЫ С КОРРЕКТНЫМ ИСПОЛЬЗОВАНИЕМ .num И .name) */}
                    <div>
